...
options:
    ...
    dim:
        description:
            - Effectively the zoom. A smaller number is a higher zoom level. Defaults to 0.4
        required: false
...
def run_module():
    ...
    module_args = dict(
        ...
        dim=dict(type='float', required=False, default=0.4),
    )
    ...
    # https://api.nasa.gov/planetary/earth/imagery/?lon=-95.21&lat=29.67&date=2018-01-01&dim=0.32&api_key=DEMO_KEY

    nasaurl = f"https://api.nasa.gov/planetary/earth/imagery/?lon={ module.params['lon'] }&lat={ module.params['lat'] }&date={ module.params['date'] }&dim={ module.params['dim'] }&api_key={ module.params['apikey'] }"
